...
The `reference_id` is **required** and must be unique per organization. It's used to link customers to your organization and is returned in the response for your records.
</Note>
...
<Tip>
The `reference_id` links the customer to your organization. To update applications later, use the PATCH endpoint with the `application_id` from the search results. PATCH requires API key authentication.
</Tip>

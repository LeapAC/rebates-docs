...
- The `customer_device_id` alias is no longer supported - use `id` only
- Devices are **update-only** - no inserts allowed
- Missing `id` will return a 400 Bad Request error

If you need to add a new device, use the appropriate creation endpoint.
</Tip>

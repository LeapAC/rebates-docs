...
<Note>
This endpoint requires API key authentication via `Authorization: Bearer <API_KEY>` or `x-api-key` header. The application must belong to your organization.

Use the `application_id` query parameter or path parameter (`PATCH /applications/{application_id}`) to identify which application to update. The API verifies the application belongs to your organization.
...
You can also use the path parameter instead of the query parameter:

- `PATCH /applications/{application_id}` - Same as using `?application_id={id}` in the query
...
- `customer_devices`: Array of devices to update (must include `id` - UPDATE ONLY)
...
```bash
curl -X PATCH \
  "https://api.leap.energy/alpha/applications?application_id=123&dryRun=true" \
...
```bash
curl -X PATCH \
  "https://api.leap.energy/alpha/applications?application_id=123" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "status": "approved",
...
{
  "code": "missing_auth",
  "message": "x-api-key header or Authorization: Bearer <API key> required",
  "requestId": "..."
}
```
...
<Tip>
**Customer Devices:** Each device in `customer_devices` must include `id`. Devices are **update-only** - no inserts allowed. If you need to add a new device, use the appropriate creation endpoint.
</Tip>

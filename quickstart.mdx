...
```http
GET /application/applications?application_status=awaiting_partner
...
Upload partner data using the reference ID from the application. The `refID` should match the `reference_id` you provided when creating the application.

```http
PATCH /application?refID=ref-12345
```
...
<Note>
The PATCH endpoint does **not** require API key authentication. It uses the anonymous key and identifies applications via the `refID` query parameter.
</Note>
